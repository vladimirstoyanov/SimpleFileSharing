...
FileSharing_GUI::FileSharing_GUI(QWidget *parent) :
    QMainWindow(parent)
    , m_aboutGUI (std::make_unique<About_GUI>())
    , m_sharedFiles (std::make_shared<SharedFiles> ())
    , m_addFileGUI (std::make_unique<AddFile_GUI>(m_sharedFiles))
    , m_defaultThreadCount (4)
    , m_downloadButtonClicked (false)
    , m_downloadColumnId(2)
    , m_clientManager (std::make_unique<ClientManager> ())
    , m_currentHost ("")
    , m_loadingGif (std::make_unique<QLabel>(this))
    , m_loadingGifWidth (16)
    , m_loadingGifHeight (16)
    , m_maxHostCount (256)
    , m_model (std::make_unique<QStandardItemModel>(0,4,this))
    , m_movie (std::make_unique<QMovie>("ajax_waiting.gif"))
    , m_nameColumnId(0)
    , m_offsetBetweenWidgets(5)
    , m_progressColumnId(1)
    , m_scanIpGUI (std::make_unique<ScanIP_GUI> ())
    , m_scanNetwork (std::make_unique<ScanNetworkThread>(m_maxHostCount))
    , m_server (m_sharedFiles)
    , m_setDirGUI (std::make_unique<SetDir_GUI> ())
    , m_setNetworkGUI (std::make_unique<SetNetwork_GUI>())
    , m_sizeColumnId (3)
